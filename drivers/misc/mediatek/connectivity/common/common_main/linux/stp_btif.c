...
INT32 mtk_wcn_consys_stp_btif_tx(UINT8 * const pBuf, const UINT32 len, UINT32 *written_len)
{
...
INT32 mtk_wcn_consys_stp_btif_parser_wmt_evt(UINT8 * const str, UINT32 len)
{
	if (!stpBtifId) {
		WMT_WARN_FUNC("NULL BTIF ID reference!\n");
		return -1;
