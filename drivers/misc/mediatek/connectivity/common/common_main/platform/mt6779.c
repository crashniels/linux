...
static INT32 polling_consys_chipid(VOID)
{
	...
	consys_reg_base = ioremap(CONSYS_IDENTIFY_ADIE_CR_ADDRESS, 0x8);
	if (consys_reg_base) {
		value = CONSYS_REG_READ(consys_reg_base);
#if CONSYS_PMIC_CTRL_6635
	...
	consys_reg_base = ioremap(CONSYS_WPLL_SETTING_ADDRESS, 0x100);
	if (consys_reg_base) {
	...
	consys_reg_base = ioremap(CONSYS_TOPPOSE_RESTORE_ADDRESS, 0x100);
	if (consys_reg_base) {
		CONSYS_REG_WRITE(consys_reg_base,
				(CONSYS_REG_READ(consys_reg_base) &
