...
/*For IDC test*/
typedef int (*CONN_MD_MSG_RX_CB) (conn_md_ipc_ilm_t *ilm);
...
//extern int mtk_conn_md_bridge_send_msg(conn_md_ipc_ilm_t *ilm);

#if 1
extern int __weak mtk_conn_md_bridge_reg(uint32 u_id, CONN_MD_BRIDGE_OPS *p_ops)
{
	pr_err(KERN_ERR "MTK_CONN Weak FUNCTION~~~\n");
	return 0;
}

extern int __weak mtk_conn_md_bridge_unreg(uint32 u_id)
{
	pr_err(KERN_ERR "MTK_CONN Weak FUNCTION~~~\n");
	return 0;
}

extern int __weak mtk_conn_md_bridge_send_msg(conn_md_ipc_ilm_t *ilm)
{
	pr_err(KERN_ERR "MTK_CONN Weak FUNCTION~~~\n");
	return 0;
}
